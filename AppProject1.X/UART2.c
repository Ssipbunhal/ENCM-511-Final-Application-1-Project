...
// display time: minutes and seconds in the format minutes m : seconds s
void DispTime(uint16_t minutes, uint16_t seconds)
{ 
    XmitUART2('\r',1);
    ...
    // if alarm is on, display alarm message at the end
    if(alarm == 1){
        Disp2String("-- ALARM");
    }
    else{
